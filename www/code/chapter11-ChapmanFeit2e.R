###################################
# Code for: R for Marketing Research and Analytics, 2nd ed: Chapter 11
#
...
#################################################################
# BRIEF HOW TO USE
# This file contains scripts used in Chapter 11 of Chapman & Feit (2019),
...
######## START digression: visualize trees

# The following function is from:
# P. Caldon, "to.dendrogram", posted Dec 2012, at http://stats.stackexchange.com/a/45161
# retrieved Apr 9, 2014
#
# This function is NOT reproduced in the book, only here for convenience
...
##### End of function from P. Caldon. 
##### Following code generates chart in the book; code is not in the book
tree1 <- getTree(seg.rf, 987, labelVar=TRUE)
d1 <- to.dendrogram(tree1)

tree2 <- getTree(seg.rf, 1245, labelVar=TRUE)
d2 <- to.dendrogram(tree2)

plot(d1, center=TRUE, edgePar=list(t.cex=0.8, p.col=NA, p.lty=0), 
     main="Example: Tree 987", yaxt="n")

plot(d2, center=TRUE, edgePar=list(t.cex=0.8, p.col=NA, p.lty=0), 
     main="Example: Tree 1245", yaxt="n")
########
######## END digression: visualize trees


# predict the test data for random forest
seg.rf.class <- predict(seg.rf, seg.df.test) # predicted classes
